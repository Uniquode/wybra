...
from wevra.auth.mfa.totp import generate_totp_secret, totp_auth_uri
from wevra.auth.options import TOTP_REQUIRED
from wevra.auth.result import ERROR_TOTP_SETUP_REQUIRED
from wevra.auth.routes.totp import (
        ...
        if authenticated_user is not None and challenge_user is not None:
            if str(authenticated_user.id) != str(challenge_user.id):
                logger.warning(
                    ...
                    log_safe_line(log_safe_uuid(authenticated_user.id)),
                    log_safe_line(log_safe_uuid(challenge_user.id)),
                    log_safe_line(log_safe_uuid(setup_challenge_id)),
                )
                setup_challenge = None
                challenge_user = None
