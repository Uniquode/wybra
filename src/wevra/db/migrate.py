...
from wevra.db.persistence import close_database, create_database_engine
from wevra.db.provisioning import (
...
REVISION_HELP = (
    ...
    "wevra-migrate upgrade; then run validation."
)
...
def _upgrade_initialised_database(config: Config, revision: str) -> None:
    ...
    if database_path is not None and not database_path.exists():
        raise MigrationStateError(
            ...
            "for first-time schema setup before using `wevra-migrate upgrade`."
        )
...
def _show_current_revision(config: Config) -> None:
    ...
    if not state.initialised:
        ...
        click.echo("hint: run `uv run wevra-migrate init` for first-time setup")
        return
