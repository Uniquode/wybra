...
from wybra.forms.csrf import CsrfField, request_csrf_response_finalisation
...
def forms_context(request: Request, context: TemplateContext) -> TemplateContext:
    ...
    csrf_renderable = csrf.get("csrf_field")
    if not isinstance(csrf_renderable, CsrfField):
        csrf_renderable = csrf
    templates = request.app.state.site.require_capability(TemplateCapability)
    return context.with_layer(csrf).with_layer(
        forms_rendering_context(templates, csrf_renderable, url_context=request),
    )
