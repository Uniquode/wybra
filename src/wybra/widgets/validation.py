...
from wybra.widgets.config import (
...
from wybra.widgets.features import LOGIN_WIDGET, THEME_WIDGET, WidgetFeature
...
def validate_widgets(settings: WidgetsValidationSettings) -> ValidationResult:
    ...
    if "wybra.widgets" not in settings.modules:
        record_check(
            checks,
            errors,
            passed=True,
            description="wybra.widgets is not configured",
        )
    ...
    record_check(
        ...
        errors,
        passed=True,
        description="wybra.widgets is configured",
    )
...
def _validate_widget_resources(
    ...
    template_source = PackageResourceSource(
        package="wybra.widgets",
        directory="templates",
    )
    static_source = PackageResourceSource(package="wybra.widgets", directory="static")
