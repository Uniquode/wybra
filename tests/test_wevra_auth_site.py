...
from wevra.auth import AuthCapability, anonymous_required, login_required
...
from wevra.site import SiteCapabilityError, start


def _site_config_source(
    tmp_path: Path,
    *,
    modules: tuple[str, ...] = ("wevra.db", "wevra.auth"),
) -> MappingConfigSource:
...
@pytest.mark.anyio
async def test_wevra_auth_setup_site_registers_auth_capability(
    tmp_path: Path,
) -> None:
    app = FastAPI()
...
@pytest.mark.anyio
async def test_wevra_auth_setup_site_requires_database_capability(
    tmp_path: Path,
) -> None:
    with pytest.raises(SiteCapabilityError, match="Missing capability"):
        await start(
            FastAPI(),
            config_source=_site_config_source(tmp_path, modules=("wevra.auth",)),
        )


@pytest.mark.anyio
async def test_wevra_auth_setup_is_omitted_when_module_is_not_configured(
    tmp_path: Path,
) -> None:
    site = await start(
        FastAPI(),
        config_source=_site_config_source(tmp_path, modules=("wevra.db",)),
    )
